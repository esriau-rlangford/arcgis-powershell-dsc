    ...
    if($MachineName -as [ipaddress]){
        ...
        {        
            ...
            Try {
                ...
                if($null -eq $DnsRecord) {
                    $computerSystemInstance = Get-CimInstance -Class Win32_ComputerSystem -ComputerName $MachineName -ErrorAction SilentlyContinue

                    $NetworkAdapter = Get-CimInstance -Class Win32_NetworkAdapterConfiguration -Filter IPEnabled=TRUE -ComputerName $MachineName -ErrorAction SilentlyContinue |
                    Where-Object { -not [string]::IsNullOrWhiteSpace($_.DNSDomain) } |
                    Select-Object -First 1

                    $hostName = $computerSystemInstance.Name
                    $dnsSuffix = if($NetworkAdapter) { $NetworkAdapter.DNSDomain } elseif($computerSystemInstance.PartOfDomain) { $computerSystemInstance.Domain } else { $null }

                    if(-not [string]::IsNullOrWhiteSpace($hostName) -and -not [string]::IsNullOrWhiteSpace($dnsSuffix)) {
                        $Dns = "$hostName.$dnsSuffix"
                        $ResolvedDns = $true
                    }
                }
            }
    ...
    while(-not($Done)) {
        ...
        ForEach($j in $Job.ChildJobs){
            ...
            if($PosErrorMap.ContainsKey($j.Name)){
                ...
                $PosErrorMap.Add($j.Name,$PosError)
            }
...
function Get-DownloadsInstallsConfigurationData
{
    ...
    if($ConfigData.ConfigData.Server){
        ...
        if($ConfigData.ConfigData.Server.LogLevel){
            $ConfigData.ConfigData.Server.Remove("LogLevel")
        }
    ...
    if($ConfigData.ConfigData.RealityServer){
        if($ConfigData.ConfigData.RealityServer.LicenseFilePath){
            $ConfigData.ConfigData.RealityServer.Remove("LicenseFilePath")
        }
        if($ConfigData.ConfigData.RealityServer.LicensePassword){
            $ConfigData.ConfigData.RealityServer.Remove("LicensePassword")
        }
    }
    if($ConfigData.ConfigData.Portal){
        ...
        if($ConfigData.ConfigData.Portal.LogLevel){
            $ConfigData.ConfigData.Portal.Remove("LogLevel")
        }
    }
    ...
    if(([version]($ConfigData.ConfigData.Version) -ge "11.5") -and $ConfigData.ConfigData.WebAdaptor){
        $ConfigData.ConfigData.WebAdaptor.Remove("AdminAccessEnabled")
    }
...
function Resolve-SecretFromObject
{
    [CmdletBinding()]
    [OutputType([System.Security.SecureString])]
    param(
    ...
    $fileProp = "$Prefix`FilePath"
    $envProp  = "$Prefix`EnvironmentVariableName"
    $plainProp= "$Prefix"

    # Helper to safely check properties on both hashtable + PSObject
    function _HasProp([object]$o, [string]$name) {
        if ($null -eq $o) { return $false }
        if ($o -is [hashtable]) { return $o.ContainsKey($name) }
        return $null -ne ($o.PSObject.Properties[$name])
    }

    function _GetProp([object]$o, [string]$name) {
        if ($o -is [hashtable]) { return $o[$name] }
        return $o.$name
    }

    if (_HasProp $Object $fileProp) {
        $path = _GetProp $Object $fileProp
        if (-not [string]::IsNullOrWhiteSpace($path)) {
            if (-not (Test-Path -Path $path)) {
                throw "Password file $path does not exist."
            }
            return (Get-Content -Path $path | ConvertTo-SecureString)
        }
    }

    if (_HasProp $Object $envProp) {
        $envName = _GetProp $Object $envProp
        if (-not [string]::IsNullOrWhiteSpace($envName)) {
            return (Get-PasswordFromEnvironmentVariable -EnvironmentVariableName $envName)
        }
    }

    if (_HasProp $Object $plainProp) {
        $plain = _GetProp $Object $plainProp
...
function Invoke-ArcGISConfiguration
{
    ...
    if(@("Install","InstallLicense","InstallLicenseConfigure","Uninstall", "WebGISDRImport", "WebGISDRExport") -icontains $Mode){
        ...
        if($ConfigurationParamsHashtable.ConfigData.Version){
            ...
            if([version]$EnterpriseVersion -lt "10.9.1"){
                throw "[ERROR] DSC Module only supports ArcGIS Enterprise 10.9.1 and above."
            }

            if([version]$EnterpriseVersion -ge "11.4"){
            ...
            if([version]$EnterpriseVersion -ge "12.0"){
                ...
                if($TileCacheDataStoreCheck){
                    throw "[ERROR] Starting at ArcGIS Enterprise 12.0, the Tile Cache Data Store is retired. Please update your configuration file."
                }
            }
        }

        if($ConfigurationParamsHashtable.ConfigData.ProVersion){
            $ProVersion = $ConfigurationParamsHashtable.ConfigData.ProVersion
            if([version]$ProVersion -lt "3.2"){
                throw "[ERROR] DSC Module only supports ArcGIS Pro 3.2 and above."
            }

            if([version]$ProVersion -gt "3.6" -and $ConfigurationParamsHashtable.ConfigData.Pro.AuthorizationType -ieq "CONCURRENT_USE"){
                throw "[ERROR] Concurrent use license type was deprecated on July 1, 2025. It is only supported with ArcGIS Pro 3.6 and below."
            }
        }
        ...
        if(@("WebGISDRImport", "WebGISDRExport") -icontains $Mode){
            if($ConfigurationParamsHashtable.ConfigData.Portal `
                ...
                $WebGISDRPortalCD.AllNodes += (Invoke-CreateNodeToAdd -Node $PortalNode -TargetComponent 'Portal')
                $WebGISDRArgs = @{
                    ConfigurationData = $WebGISDRPortalCD
        ...
        if($JobFlag[$JobFlag.Count - 1] -eq $True -and ($Mode -ieq "InstallLicense" -or $Mode -ieq "InstallLicenseConfigure")){
            ...
            if(-not($ServerSkipLicenseStep -and $ProSkipLicenseStep -and $LicenseManagerSkipLicenseStep)){
                $LicenseCD = @{
                    AllNodes = @() 
                }
                ...
                for ( $i = 0; $i -lt $ConfigurationParamsHashtable.AllNodes.count; $i++ ){
                    ...
                    if($Node.Role -icontains "Server"){
                        ...
                        if($ServerRole -ine "GeoEvent" -and  $ServerRole -ine "WorkflowManagerServer"){
                            ...
                            $ServerLicensePassword = Resolve-SecretFromObject -Object $ConfigurationParamsHashtable.ConfigData.Server -Prefix 'LicensePassword'
                            
                            if($Node.ServerLicenseFilePath){
                                $ServerLicenseFilePath = $Node.ServerLicenseFilePath
                                $ServerLicensePassword = Resolve-SecretFromObject -Object $Node -Prefix 'ServerLicensePassword'
                            }
                        ...
                        if(($ServerRole -ieq "GeoEvent" -or ($ServerRole -ieq "GeneralPurposeServer" -and $ConfigurationParamsHashtable.ConfigData.AdditionalServerRoles -icontains "GeoEvent")) -and $ConfigurationParamsHashtable.ConfigData.GeoEventServer){
                            ...
                            $GeoeventServerLicensePassword = Resolve-SecretFromObject -Object $ConfigurationParamsHashtable.ConfigData.GeoEventServer -Prefix 'LicensePassword'
                            if($Node.GeoeventServerLicenseFilePath)
                            {
                                $GeoeventServerLicenseFilePath =  $Node.GeoeventServerLicenseFilePath
                                $GeoeventServerLicensePassword = Resolve-SecretFromObject -Object $Node -Prefix 'GeoeventServerLicensePassword'
                            }
                        ...
                        if(($ServerRole -ieq "WorkflowManagerServer" -or ($ServerRole -ieq "GeneralPurposeServer" -and $ConfigurationParamsHashtable.ConfigData.AdditionalServerRoles -icontains "WorkflowManagerServer")) -and $ConfigurationParamsHashtable.ConfigData.WorkflowManagerServer){
                            ...
                            $WorkflowManagerServerLicensePassword = Resolve-SecretFromObject -Object $ConfigurationParamsHashtable.ConfigData.WorkflowManagerServer -Prefix 'LicensePassword'
                            ...
                            if($Node.WorkflowManagerServerLicenseFilePath)
                            {
                                ...
                                $WorkflowManagerServerLicensePassword = Resolve-SecretFromObject -Object $Node -Prefix 'WorkflowManagerServerLicensePassword'
                            }
                        ...
                        if($ServerRole -ieq "RealityServer" -and $ConfigurationParamsHashtable.ConfigData.RealityServer){
                            $RealityServerLicenseFilePath =  $ConfigurationParamsHashtable.ConfigData.RealityServer.LicenseFilePath
                            $RealityServerLicensePassword = Resolve-SecretFromObject -Object $ConfigurationParamsHashtable.ConfigData.RealityServer -Prefix 'LicensePassword'
                            # Per Node - RealityServer
                            if($Node.RealityServerLicenseFilePath)
                            {
                                $RealityServerLicenseFilePath = $Node.RealityServerLicenseFilePath
                                $RealityServerLicensePassword = Resolve-SecretFromObject -Object $Node -Prefix 'RealityServerLicensePassword'
                            }

                            $NodeToAdd["RealityServerLicenseFilePath"] = $RealityServerLicenseFilePath
                            if($null -ne $RealityServerLicensePassword){
                                $NodeToAdd["RealityServerLicensePassword"] = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ("PlaceHolder", $RealityServerLicensePassword)
                            }
                        }
                    }
                    if($Node.Role -icontains "Pro"){
                        ...
                        if($ConfigurationParamsHashtable.ConfigData.Pro.AuthorizationType -ieq "SINGLE_USE"){
                            ...
                            if($Node.ProLicenseFilePath)
                            {
                                ...
                                $ProLicensePassword = Resolve-SecretFromObject -Object $Node -Prefix 'ProLicensePassword'
                            }
            ...
            if(($ProCheck -or $LicenseManagerCheck) -and -not($ServerCheck -or $PortalCheck)){
            ...
            if($JobFlag[$JobFlag.Count - 1] -eq $True -and ($Mode -ieq "InstallLicenseConfigure") -and -not($SkipConfigureStep)){
                ...
                if($IsHAServer) {
                    if($ConfigurationParamsHashtable.ConfigData.Server){
                        if($MappedDriveOverrideFlag -or ((@("NotebookServer","MissionServer","VideoServer","DataPipelinesServer") -inotcontains $ServerRole) -and $ConfigurationParamsHashtable.ConfigData.Server.CloudServices)){
                                ...
                                if($ConfigurationParamsHashtable.ConfigData.Server.ServerDirectoriesRootLocation.StartsWith('\')){
                                    ...
                                    if($ConfigurationParamsHashtable.ConfigData.Server.ServerDirectories){
                                        foreach($dir in $ConfigurationParamsHashtable.ConfigData.Server.ServerDirectories){ 
                                            ...
                                            $ServerDirPathToTest = if(@("NotebookServer","MissionServer","VideoServer","DataPipelinesServer") -icontains $ServerRole){ $dir.path }else{ $dir.physicalPath }
                ...
                if(-not($ValidateServerFileShare -and $ValidatePortalFileShare)){
                ...
                for ( $i = 0; $i -lt $ConfigurationParamsHashtable.AllNodes.count; $i++ ){
                    ...
                    if($Node.Role -icontains 'WebAdaptor'){
                        ...
                        $WebAdaptorAdminAccessEnabledSupported = -not(@("NotebookServer","MissionServer","VideoServer","DataPipelinesServer") -iContains $ConfigurationParamsHashtable.ConfigData.ServerRole)
                        if([version]$EnterpriseVersion -ge "11.5"){
                            # AdminAccessEnabled flag is not honored from version 11.5 onwards. Defaulting it to True
                ...
                if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $ServerCheck){
                    ...
                    if($ConfigurationParamsHashtable.ConfigData.Server.LogLevel){
                        $ServerArgs["LogLevel"] = $ConfigurationParamsHashtable.ConfigData.Server.LogLevel
                    }

                    # Cloud Native Server
                    if($ConfigurationParamsHashtable.ConfigData.Server.CloudServices){
                        if([version]$EnterpriseVersion -le "11.5"){
                                    ...
                                    if($AzureCloudProviderConfig.Credential){
                                        if(@("ServicePrincipal","UserAssignedIdentity") -icontains $AzureCloudProviderConfig.Credential.Type){
                                            ...
                                            if($AzureCloudProviderConfig.Credential.Type -ieq "ServicePrincipal"){
                                                $ServicePrincipalClientSecret = Resolve-SecretFromObject -Object $AzureCloudProviderConfig.Credential.ServicePrincipal -Prefix 'ClientSecret'
                                        ...
                                        if($AzureCloudProviderConfig.StorageAccount.Credential){
                                            if(@("StorageAccountKey") -icontains $AzureCloudProviderConfig.StorageAccount.Credential.Type){
                                                if($AzureCloudProviderConfig.StorageAccount.Credential.Type -ieq "StorageAccountKey"){
                                                    $AccountKeySecret = Resolve-SecretFromObject -Object $AzureCloudProviderConfig.StorageAccount.Credential.StorageAccountKey -Prefix 'AccountKey'
                                        ...
                                        if($AzureCloudProviderConfig.CosmosDBAccount.Credential){
                                            if(@("CosmosDBAccountKey") -icontains $AzureCloudProviderConfig.CosmosDBAccount.Credential.Type){
                                                if($AzureCloudProviderConfig.CosmosDBAccount.Credential.Type -ieq "CosmosDBAccountKey"){
                                                    $AccountKeySecret = Resolve-SecretFromObject -Object $AzureCloudProviderConfig.CosmosDBAccount.Credential.CosmosDBAccountKey -Prefix 'AccountKey'
                                        ...
                                        if($AzureCloudProviderConfig.ServiceBusNamespace.Credential){
                                            if(@("ServiceBusNamespaceAccessKey") -icontains $AzureCloudProviderConfig.ServiceBusNamespace.Credential.Type){
                                                if($AzureCloudProviderConfig.ServiceBusNamespace.Credential.Type -ieq "ServiceBusNamespaceAccessKey"){
                                                    $SharedAccessKeySecret = Resolve-SecretFromObject -Object $AzureCloudProviderConfig.ServiceBusNamespace.Credential.ServiceBusNamespaceAccessKey -Prefix 'SharedAccessKey'
                                    ...
                                    if($AWSCloudProviderConfig.Credential){
                                        if(@("AccessKey","IAMRole") -icontains $AWSCloudProviderConfig.Credential.Type){
                                            ...
                                            if($AWSCloudProviderConfig.Credential.Type -ieq "AccessKey"){
                                                $AccessKeySecret = Resolve-SecretFromObject -Object $AWSCloudProviderConfig.Credential -Prefix 'SecretKey'
                        ...
                        if($ConfigurationParamsHashtable.ConfigData.Server.ConfigStoreCloudStorageAccount){
                            ...
                            if($ConfigCloudStorageType -ieq "AWSS3DynamoDB"){
                                ...
                                if($ConfigStoreAzureBlobAuthenticationType -ieq "UserAssignedIdentity"){
                                    ...
                                    $ConfigStoreAzureBlobServicePrincipalClientSecret = Resolve-SecretFromObject -Object $ConfigurationParamsHashtable.ConfigData.Server.ConfigStoreCloudStorageAccount.AzureBlobServicePrincipal -Prefix 'ClientSecret'
                    ...
                    if(@("NotebookServer", "MissionServer", "VideoServer","DataPipelinesServer") -iContains $ConfigurationParamsHashtable.ConfigData.ServerRole){
                        $ConfigurationName = "ArcGISNonGISServer"
                        $ServerArgs["ServerType"] = $ConfigurationParamsHashtable.ConfigData.ServerRole
                        if($ConfigurationParamsHashtable.ConfigData.ServerRole -eq "NotebookServer"){
                            if($ConfigurationParamsHashtable.ConfigData.Server.ContainerImagePaths){
                                $ServerArgs["NotebookServerContainerImagePaths"] = $ConfigurationParamsHashtable.ConfigData.Server.ContainerImagePaths
                            }        
                            
                            if($ConfigurationParamsHashtable.ConfigData.Server.Installer.NotebookServerSamplesDataPath){
                                if(@("10.9.1","11.0","11.1","11.2","11.3") -icontains $EnterpriseVersion){
                                    $ServerArgs["ExtractNotebookServerSamplesData"] = $True
                                }else{
                                    throw "Notebook Server Samples Data Path is not supported for $EnterpriseVersion."
                                }
                            }
                        }

                        if($ConfigurationParamsHashtable.ConfigData.ServerRole -eq "VideoServer"){
                            if($ConfigurationParamsHashtable.ConfigData.Server.LiveStreamPorts){
                                $ServerArgs["VideoServerLiveStreamPorts"] = ($ConfigurationParamsHashtable.ConfigData.Server.LiveStreamPorts | ConvertTo-Json)
                            }
                        }
                    }else{
                        if($ConfigurationParamsHashtable.ConfigData.ServerRole -ieq "GeoEventServer"){
                        ...
                        if ($ConfigurationParamsHashtable.ConfigData.Server.SocMaximumHeapSize) {
                            ...
                            $ServerArgs["SocMaximumHeapSize"] =$ParsedHeap
                        }
                    }
                ...
                if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $ConfigurationParamsHashtable.ConfigData.Server.DataStoreItems){
                    if([version]$EnterpriseVersion -lt "11.2"){
                    ...
                    if(($ObjectStoresObject | Measure-Object).Count -gt 0){
                        ...
                        if([version]$EnterpriseVersion -lt "11.4"){
                ...
                {
                    $ServerSettingsArgs = @{
                        ConfigurationData = @{ AllNodes = @($PrimaryServerMachine) } 
                        ServerPrimarySiteAdminCredential = $ServerPrimarySiteAdminCredential
                        PrimaryServerMachine = $PrimaryServerMachine.NodeName
                    ...
                    if(@("NotebookServer", "MissionServer", "VideoServer","DataPipelinesServer") -icontains $ConfigurationParamsHashtable.ConfigData.ServerRole){
                        $ServerSettingsArgs["ServerType"] = $ConfigurationParamsHashtable.ConfigData.ServerRole
                        ...
                        if($ConfigurationParamsHashtable.ConfigData.ServerRole -ieq "VideoServer"){
                            if($ConfigurationParamsHashtable.ConfigData.Server.LiveStreamGatewayHostName){
                                $ServerSettingsArgs["VideoServerLiveStreamGatewayHostName"] = $ConfigurationParamsHashtable.ConfigData.Server.LiveStreamGatewayHostName
                            }

                            if($ConfigurationParamsHashtable.ConfigData.Server.LiveStreamPorts){
                                $ServerSettingsArgs["VideoServerLiveStreamPorts"] = ($ConfigurationParamsHashtable.ConfigData.Server.LiveStreamPorts | ConvertTo-Json)
                            }
                        }
                    }else{
                        $ServerSettingsArgs["ServerType"] = "Server"
                ...
                if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $PortalCheck){
                    ...
                    if($ConfigurationParamsHashtable.ConfigData.Portal.LogLevel){
                        $PortalArgs["LogLevel"] = $ConfigurationParamsHashtable.ConfigData.Portal.LogLevel
                    }

                    $PortalAdminSecurityQuestionPassword = Resolve-SecretFromObject -Object $ConfigurationParamsHashtable.ConfigData.Portal.PortalAdministrator -Prefix 'SecurityAnswer'
                    ...
                    if($ConfigurationParamsHashtable.ConfigData.Portal.PortalContentCloudStorageAccount){
                        if($ConfigurationParamsHashtable.ConfigData.Portal.PortalContentCloudStorageAccount.CloudStorageType -ieq "AzureBlob"){
                            ...
                            if($ContentStoreAzureBlobAuthenticationType -ieq "UserAssignedIdentity"){
                                ...
                                $ConfigStoreAzureBlobServicePrincipalClientSecret = Resolve-SecretFromObject -Object $ConfigurationParamsHashtable.ConfigData.Portal.PortalContentCloudStorageAccount.AzureBlobServicePrincipal -Prefix 'ClientSecret'
                ...
                if($PortalCheck -and ($JobFlag[$JobFlag.Count - 1] -eq $True)){
                    foreach($PortalNode in $PortalCD.AllNodes){
                        if($JobFlag[$JobFlag.Count - 1] -eq $True){
                            $JobFlag = $False
                            $PortalArgs = @{
                                ConfigurationData = @{ AllNodes = @($PortalNode) }
                                Version = $EnterpriseVersion
                                PortalAdministratorCredential = $PortalAdministratorCredential
                            ...
                            $JobFlag = Invoke-DSCJob -ConfigurationName "ArcGISPortalTLSSettings" -ConfigurationFolderPath "Configurations-OnPrem" -Arguments $PortalArgs -Credential $Credential -UseWinRMSSL $UseWinRMSSL -DebugMode $DebugMode
                        }
                    }
                }
                ...
                if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $PortalCheck){
                    $PortalSettingsArgs = @{
                        ConfigurationData = @{ AllNodes = @($PrimaryPortalMachine)}
                        Version = $EnterpriseVersion
                        PrimaryPortalMachine = $PrimaryPortalMachine.NodeName
                        PortalAdministratorCredential = $PortalAdministratorCredential
                ...
                if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $GraphDataStoreCheck){
                    ...
                    elseif([version]$EnterpriseVersion -ge "11.5"){ 
                ...
                if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $ObjectDataStoreCheck){
                    ...
                    if(([version]$EnterpriseVersion -ge "11.2") -and `
                ...
                if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $ServerCheck){
                    ...
                    $FederationConfig = $ConfigurationParamsHashtable.ConfigData.Federation
                    if($null -ne $FederationConfig){
                        $Federation = $True
                        $PortalHostName = $FederationConfig.PortalHostName
                        $PortalPort = $FederationConfig.PortalPort
                        $PortalContext = $FederationConfig.PortalContext

                        $RemoteSiteAdministratorPassword = (Get-PasswordFromObject -Object $FederationConfig.PortalAdministrator)
                        $RemoteSiteAdministrator = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ( $FederationConfig.PortalAdministrator.UserName, $RemoteSiteAdministratorPassword )

                        $RequiredFederationServerKeys = @(
                            'ServerHostName'
                            'ServerUrlPort'
                            'ServerContext'
                            'ServerAdminHostName'
                    ...
                    if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $GeoenrichmentServerCheck){
                        ...
                        $JobFlag = $False
                        $ArcGISGeoEnrichmentServerItemArgs = @{
                            ConfigurationData = $ServerCD
                            Version = $EnterpriseVersion
                            PortalAdministratorCredential = $RemoteSiteAdministrator
                            DataStoreDataDirectory = if($ConfigurationParamsHashtable.ConfigData.GeoEnrichmentServer.DataStoreDataDirectory){ $ConfigurationParamsHashtable.ConfigData.GeoEnrichmentServer.DataStoreDataDirectory }else{ $null }
                            ForceRepair = if($ConfigurationParamsHashtable.ConfigData.GeoEnrichmentServer.ForceRepair){ $ConfigurationParamsHashtable.ConfigData.GeoEnrichmentServer.ForceRepair }else{ $False }
                            RegisterGeoEnrichmentAsPortalUtilityService = if($ConfigurationParamsHashtable.ConfigData.GeoEnrichmentServer.RegisterGeoEnrichmentAsPortalUtilityService){ $ConfigurationParamsHashtable.ConfigData.GeoEnrichmentServer.RegisterGeoEnrichmentAsPortalUtilityService }else{ $True }
                        }

                        $JobFlag = Invoke-DSCJob -ConfigurationName "ArcGISGeoEnrichmentServer" -ConfigurationFolderPath "Configurations-OnPrem" -Arguments $ArcGISGeoEnrichmentServerItemArgs -Credential $Credential -UseWinRMSSL $UseWinRMSSL -DebugMode $DebugMode
                    }
                }
                ...
                if($JobFlag[$JobFlag.Count - 1] -eq $True){ 
                    ...
                    if($ServerCheck -and $null -ne $ServerDetails ){
                        ...
                        if(-not($ConfigurationParamsHashtable.ConfigData.ServerRole -in @('MissionServer', 'NotebookServer','VideoServer','DataPipelinesServer'))){
        ...
        foreach($cf in $ConfigurationParametersFile){
            ...
            $OldVersion = $cfHashtable.ConfigData.OldVersion

            if([version]$OldVersion -lt "10.9.1"){
                throw "[ERROR] DSC Module only supports upgrades from ArcGIS Enterprise 10.9.1 and above. Configuration File Name - $cf"
            }
            ...
            if([version]$OldVersion -lt "11.0" -and [version]$UpgradeVersion -ge "12.0"){
            ...
            if([version]$UpgradeVersion -ge "11.4"){
            ...
            if([version]$UpgradeVersion -ge "12.0"){
            ...
            if ($cfHashtable.ConfigData.WebAdaptor -and $cfHashtable.ConfigData.WebAdaptor.IsJavaWebAdaptor) {
                ...
                -and $cfHashtable.ConfigData.WebAdaptor.Installer.ApacheTomcat) 
                {
                    ...
                    -EnterpriseVersion $UpgradeVersion 
                }
            }
...
function Invoke-DataStoreConfigureScript
{
    ...
    $EnterpriseVersion = $DataStoreArguments.Version
        ...
        for ( $i = 0; $i -lt $Backups.Count; $i++ ){
            ...
            if($BackupObject.Type -ine "fs")
            {
                if($BackupObject.CloudStorageAccount){
                    ...
                    if($BackupObject.Type -ieq "s3" -and ([version]$EnterpriseVersion -ge "11.2")){
...
function Invoke-PortalUpgradeScript {
    ...
    if($PortalConfig.ConfigData.Portal.Installer.WebStylesPath){
    ...
    if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $HasPortalWANodes){
        ...
        if($IsJavaWebAdaptor){
            ...
            $WebAdaptorUpgradeArgs["DotnetHostingBundlePath"] = if([version]$Version -ge "11.5"){ $PortalConfig.ConfigData.WebAdaptor.Installer.DotnetHostingBundlePath }else{ $null }
            $WebAdaptorUpgradeArgs["WebDeployPath"] = if([version]$Version -ge "11.5"){ $PortalConfig.ConfigData.WebAdaptor.Installer.WebDeployPath }else{ $null }
        }
...
function Invoke-ServerUpgradeScript {
    ...
    for ( $i = 0; $i -lt $cf.AllNodes.count; $i++ ){
        ...
        if($Node.Role -icontains 'Server'){
            ...
            if($ServerRole -ine "GeoEvent" -and $ServerRole -ine "WorkflowManagerServer"){
                ...
                $ServerLicensePassword = Resolve-SecretFromObject -Object $cf.ConfigData.Server -Prefix 'LicensePassword'
            ...
            if(($ServerRole -ieq "GeoEvent" -or ($ServerRole -ieq "GeneralPurposeServer" -and $AdditionalServerRoles -icontains "GeoEvent")) -and $cf.ConfigData.GeoEventServer){
                ...
                $GeoeventServerLicensePassword = Resolve-SecretFromObject -Object $cf.ConfigData.GeoEventServer -Prefix 'LicensePassword'
                if($Node.GeoeventServerLicenseFilePath)
                {
                    $GeoeventServerLicenseFilePath =  $Node.GeoeventServerLicenseFilePath
                    $GeoeventServerLicensePassword = Resolve-SecretFromObject -Object $Node -Prefix 'GeoeventServerLicensePassword'
            ...
            if(($ServerRole -ieq "WorkflowManagerServer" -or ($ServerRole -ieq "GeneralPurposeServer" -and $AdditionalServerRoles -icontains "WorkflowManagerServer")) -and $cf.ConfigData.WorkflowManagerServer){
                ...
                $WorkflowManagerServerLicensePassword = Resolve-SecretFromObject -Object $cf.ConfigData.WorkflowManagerServer -Prefix 'LicensePassword'
                # Per Node - WorkflowManager
                if($Node.WorkflowManagerServerLicenseFilePath)
                {
                    $WorkflowManagerServerLicenseFilePath = $Node.WorkflowManagerServerLicenseFilePath
            ...
            $ServerUpgradeArgs = @{
                ...
                EnableDotnetSupport = if([version]$Version -ge "10.9.1"){ if($cf.ConfigData.Server.Installer.ContainsKey("EnableDotnetSupport")){ $cf.ConfigData.Server.Installer.EnableDotnetSupport } else { $True } } else { $False }
                ...
                DotnetDesktopRuntimePath = if([version]$Version -ge "10.9.1"){ if($cf.ConfigData.Server.Installer.ContainsKey("DotnetDesktopRuntimePath")){ $cf.ConfigData.Server.Installer.DotnetDesktopRuntimePath } else { $null } } else { $null }
                DebugMode = $DebugMode
            }
            ...
            if($ServerRole -ieq "NotebookServer"){
                ...
                if($cf.ConfigData.Server.Installer.NotebookServerSamplesDataPath){
                    if(@("10.9.1","11.0","11.1","11.2","11.3") -icontains $cf.ConfigData.Version){
                        ...
                        throw "Notebook Server Samples Data Path is only supported for 10.9.1, 11.0, 11.1, 11.2, 11.3."
                    }
                }
            }
    ...
    if(($JobFlag[$JobFlag.Count - 1] -eq $True) -and $HasServerWANodes){
        ...
        $WebAdaptorAdminAccessEnabledSupported = -not($cf.ConfigData.ServerRole -ieq "NotebookServer" -or $cf.ConfigData.ServerRole -ieq "MissionServer" -or $cf.ConfigData.ServerRole -ieq "VideoServer"  -or $cf.ConfigData.ServerRole -ieq "DataPipelinesServer")
        ...
        if($IsJavaWebAdaptor){
            ...
            $WebAdaptorUpgradeArgs["DotnetHostingBundlePath"] = if([version]$Version -ge "11.0"){ $cf.ConfigData.WebAdaptor.Installer.DotnetHostingBundlePath }else{ $null }
            $WebAdaptorUpgradeArgs["WebDeployPath"] = if([version]$Version -ge "11.0"){ $cf.ConfigData.WebAdaptor.Installer.WebDeployPath }else{ $null }
        }
...
function Invoke-DataStoreUpgradeScript {
    ...
    for ( $i = 0; $i -lt $DSConfig.AllNodes.count; $i++ ){
        ...
        if($DSNode.Role -icontains 'DataStore'){
            ...
            if($DsTypes -icontains "ObjectStore")
            {
                $NodeToAdd["HasObjectStore"] = $true
                $NodeToAdd["HasMultiMachineObjectStore"] = (($DSConfig.AllNodes | Where-Object { $_.DataStoreTypes -icontains 'ObjectStore' }  | Measure-Object).Count -gt 1)
...
function Restart-ArcGISService
{
    ...
    if($RestartDelay -gt 0){
        Write-Verbose "Restarting $ServiceName Service in $RestartDelay Seconds"
...
function Wait-ForServiceToReachDesiredState
{
    ...
    if($DesiredState -ieq "Running"){
        Write-Verbose "Starting service '$ServiceName'"
        Start-Service $ServiceName 
    }

    if($DesiredState -ieq "Stopped"){
        Write-Verbose "Stopping the service '$ServiceName'"    
        Stop-Service -Name $ServiceName -ErrorAction Ignore -Force
    }
            ...
            if($key.EndsWith('}')){
                ...
                if($DisplayName -imatch [regex]::escape($ProductName) -and $Publisher -ieq "Environmental Systems Research Institute, Inc."){
                    if(($ProductName -ieq "ArcGIS Notebook Server" -and -not($DisplayName -imatch "Samples Data")) `
                        -or (($ProductName -ieq "portal" -or $ProductName -ieq "Portal for ArcGIS") -and -not($DisplayName -imatch "Web Styles")) `
                        ...
                        -or ($ProductName -ine "portal" -and $ProductName -ine "Portal for ArcGIS" -and $ProductName -ine "ArcGIS Notebook Server" -and $ProductName -ine "ArcGIS Server" -and $ProductName -ine "ArcGIS Pro")
                        ){
...
function Get-ArcGISEnterpriseComponentPort{
    ...
    switch ($ComponentName) {
        'Portal' { return 7443 }
        'Server' { return 6443 }
        'DataStore' { return 2443 }
        'MissionServer' { return 20443 }
        'NotebookServer' { return 11443 }
        'VideoServer' { 21443 }
        'DataPipelinesServer' { 14443 }
        'GeoEventServer' { 6143 }
        'Monitor' { 30443 }
...
function Get-ServerEndpointDetails {
    ...
    $ServerRolesWithAdminAlwaysEnabled = @('NotebookServer', 'MissionServer', 'VideoServer', 'DataPipelinesServer')
    $AdminAlwaysEnabledVersions = @('11.5', '12.0', '12.1')

    $AdminAccessCheck =
        (-not $WebAdaptorCheck) -or
        $ConfigData.WebAdaptor.AdminAccessEnabled -or
        ($ServerRolesWithAdminAlwaysEnabled -icontains $ServerRole) -or
        ($AdminAlwaysEnabledVersions -icontains $ConfigData.Version)

    $ComponentName = if($ServerRolesWithAdminAlwaysEnabled -icontains $ServerRole){ $ServerRole }else{ "Server" }
    
    $DefaultAdminPort = Get-ArcGISEnterpriseComponentPort -ComponentName $ComponentName

    $HasExternalDnsParam = ($null -ne $ServerExternalDNSHostName) # preserves original null-check behavior
    $HasExternalDnsValue = -not [string]::IsNullOrEmpty($ServerExternalDNSHostName)

    $ServicePort = $DefaultAdminPort
    $ServiceContext = 'arcgis'
    $AdminContext = 'arcgis'

    if ($HasExternalDnsParam) {
        $ServicePort = 443
        if (-not [string]::IsNullOrEmpty($ConfigData.ServerContext)) {
            $ServiceContext = $ConfigData.ServerContext
        }
    }

    $PrimaryFqdn = Get-FQDN $PrimaryServerMachine.NodeName

    $CertCName = $null
    if ($PrimaryServerMachine.SSLCertificate) {
        $CertCName = $PrimaryServerMachine.SSLCertificate.CName
    }

    $DefaultServiceHost = if ($HasExternalDnsValue) {
        $ServerExternalDNSHostName
    } elseif (-not [string]::IsNullOrEmpty($CertCName)) {
        $CertCName
    } else {
        $PrimaryFqdn
    }

    $DefaultAdminHost = if (-not([string]::IsNullOrWhiteSpace($CertCName))) {
        $CertCName
    } else {
        $PrimaryFqdn
    }
    ...
    $Result = @{
        ServerServiceURL       = $DefaultServiceHost
        ServerServiceURLPort   = $ServicePort
        ServerServiceURLContext= $ServiceContext
        ServerSiteAdminURL     = $DefaultAdminHost
        ServerSiteAdminURLPort = $DefaultAdminPort
        ServerSiteAdminContext = $AdminContext
    }
    
    # Check Priority 3: External Load Balancer
    $ExternalLB = $ConfigData.Server.ExternalLoadBalancer
    if (-not [string]::IsNullOrEmpty($ExternalLB)) {
        $Result.ServerServiceURL        = $ExternalLB
        $Result.ServerServiceURLPort    = 443
        $Result.ServerServiceURLContext = $ConfigData.ServerContext

        if($AdminAccessCheck) {
            $Result.ServerSiteAdminURL      = $ExternalLB
            $Result.ServerSiteAdminURLPort = 443
            $Result.ServerSiteAdminContext = $ConfigData.ServerContext
        }
    }
    elseif ($AdminAccessCheck -and $HasExternalDnsValue) {
    ...
    $InternalLB = $ConfigData.Server.InternalLoadBalancer
    if (-not [string]::IsNullOrEmpty($InternalLB)) {
        # internal load balance is used to set admin port and admin url
        $Result.ServerSiteAdminURL      = $InternalLB
        ...
        $Result.ServerSiteAdminContext  = $AdminContext
    }
...
function New-NotebookWorkspaceSMBGlobalMapping
{
    ...
    Restart-ArcGISService -ComponentName "NotebookServer" -Verbose
}
...
function Test-IsValidTomcatConfig {
    ...
    if ($TomcatVersion) {
        if ([version]$EnterpriseVersion -lt "11.5") {
            if (([version]$TomcatVersion).Major -ne 9) {
                throw "[ERROR] For ArcGIS Enterprise version $EnterpriseVersion, only Apache Tomcat version 9.x is supported."
            }
        }
        elseif ([version]$EnterpriseVersion -eq "11.5") {
            if (([version]$TomcatVersion).Major -ne 9 -and (([version]$TomcatVersion).Major -ne 10)) {
                throw "[ERROR] For ArcGIS Enterprise version 11.5, Apache Tomcat version must be either 9.x or 10.x."
            }
        }
        elseif ([version]$EnterpriseVersion -gt "11.5") {
            if (([version]$TomcatVersion).Major -ne 10) {
...
function Get-ArcGISServiceName {
    ...
    switch ($ComponentName) {
        {@('Portal','Portal for ArcGIS') -icontains $_} { return 'Portal for ArcGIS' }
        {@('Server','ArcGIS Server') -icontains $_} { return 'ArcGIS Server' }
        {@('DataStore','ArcGIS Data Store') -icontains $_} { return 'ArcGIS Data Store' }
        {@('MissionServer','ArcGIS Mission Server') -icontains $_} { return 'ArcGIS Mission Server' }
        {@('NotebookServer','ArcGIS Notebook Server') -icontains $_} { return 'ArcGIS Notebook Server' }
        {@('VideoServer','ArcGIS Video Server') -icontains $_} { return 'ArcGIS Video Server' }
        {@('DataPipelinesServer', 'ArcGIS Data Pipelines Server') -icontains $_} { return 'ArcGIS Data Pipelines Server' }
        {@('ArcGISGeoEvent') -icontains $_} { return 'ArcGISGeoEvent' }
        {@('WorkflowManager') -icontains $_} { return 'WorkflowManager' }
        default { throw "Unknown component name: $ComponentName" }
    }
}
...
Export-ModuleMember -Function Get-FQDN, Invoke-ArcGISConfiguration, Invoke-PublishWebApp, `
                                ...
                                Get-ArcGISProductDetails, Restart-ArcGISService, `
                                Get-ArcGISServiceName, Wait-ForServiceToReachDesiredState, `
                                New-NotebookWorkspaceSMBGlobalMapping, `
                                Get-ArcGISEnterpriseComponentPort
